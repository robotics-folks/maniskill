...
def add_visuals_to_sapien_link(
    ...
    for idx in range(len(visual_specs)):
        ...
        vis_name = (
            vis_spec.name if vis_spec.name != "" else f"{mj_body_name}_visual_{idx}"
        )
...
class MjcfAssetArticulationLoader:
    def __init__(
        self, scene: ManiSkillScene | None = None, use_colliders_as_visuals: bool = True
    ):
    ...
    def set_scene(self, scene: ManiSkillScene) -> MjcfAssetArticulationLoader:
        self._scene = scene
        return self
    ...
    def load_from_spec(
        ...
        assert (
            mj_root_body is not None
        ), "Something went wrong when loading articulated object"
        ...
        has_freejoint = any(
            jnt.type == mj.mjtJoint.mjJNT_FREE for jnt in mj_root_body.joints
        )
    ...
    def _parse_body(
        ...
        ), "A valid mjspec object is required for the loader"
        assert (
            self._model_dir is not None
        ), "A valid path to the folder containing the model should be provided"

        link_body_name = (
            mjs_body.name if mjs_body.name != "" else f"link_{self._num_links}"
        )
        ...
        for jnt_spec in mjs_body.joints:
            ...
            limits = (
                ...
                if self._spec.compiler.degree
                and jnt_spec.type == mj.mjtJoint.mjJNT_HINGE
                else jnt_spec.range
            )
            joints_info.append(
                MjcfJointInfo(
        ...
        for i, jnt_info in enumerate(joints_info):
            ...
            jnt_range = (
                [jnt_range_min, jnt_range_max] if jnt_limited else [-np.inf, np.inf]
            )
            ...
            match jnt_info.type:
                case "hinge":
                    link_builder.set_joint_properties(
                        ...
                        friction=jnt_info.frictionloss,  # pyright: ignore [reportArgumentType]
                        damping=jnt_info.damping,  # pyright: ignore [reportArgumentType]
                    )
                case "slide":
                    link_builder.set_joint_properties(
                        "prismatic",
