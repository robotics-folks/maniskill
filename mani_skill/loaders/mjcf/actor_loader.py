...
def add_visuals_to_actor_builder(
    ...
    for vis_spec in visual_specs:
        tf_geom_to_body = Pose(
            p=tuple(vis_spec.pos), q=tuple(get_orientation(vis_spec))
        )
        match vis_spec.type:
...
def add_colliders_to_actor_builder(
    ...
    for col_spec in colliders_specs:
        tf_geom_to_body = Pose(
            p=tuple(col_spec.pos), q=tuple(get_orientation(col_spec))
        )
...
class MjcfAssetActorLoader:
    def __init__(self, scene: ManiSkillScene | None = None):
    ...
    def set_scene(self, scene: ManiSkillScene) -> MjcfAssetActorLoader:
        self._scene = scene
        return self
    ...
    def load_from_spec(
        ...
        assert mj_root_body, "Something went wrong when loading an actor"
